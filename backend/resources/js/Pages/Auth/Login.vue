...
const form = useForm({ email: '', password: '', remember: false, next: props.next });
const submit = () => form.post('/login');
const googleHref = props.next ? `/auth/google?next=${encodeURIComponent(props.next)}` : '/auth/google';
const registerHref = props.next ? `/register?next=${encodeURIComponent(props.next)}` : '/register';
</script>
...
<template>
    ...
    <div class="flex min-h-screen flex-col items-center justify-center bg-paper-200 px-4 py-10">
        ...
        <div class="w-full max-w-sm rounded-2xl border border-paper-300 bg-white p-7 shadow-xl shadow-navy/5">
            ...
            <p class="mt-5 text-center text-sm text-ink/55">New here? <Link :href="registerHref" class="font-semibold text-brand-700 hover:underline">Create an account</Link></p>
        </div>
    </div>
</template>
